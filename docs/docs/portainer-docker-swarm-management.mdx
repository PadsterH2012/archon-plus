...
**SSH Access**: Use `hl-dswarm1` (10.202.70.20) as the primary access point for all swarm management operations.
...
#### Accessing Containers on Specific Nodes

When containers are running on worker nodes, you may need to access them directly:

```bash
# Find which node a service is running on
docker service ps <service-id> --format "{{.Node}}"

# Access the specific node where container is running
# Example: If service is on hl-dswarm2
sshpass -p 'P0w3rPla72012@@' ssh -o StrictHostKeyChecking=no paddy@10.202.70.21

# Then access the container directly
