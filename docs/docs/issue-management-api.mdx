---
title: Issue Management API Integration
description: Python and FastAPI integration examples for the Issue Management Database
sidebar_position: 9
---
...
Complete Python integration examples for the Issue Management Database, designed for AI agents and applications.
...
<Tabs>
<TabItem value="basic-class" label="Basic IssueManager" default>
...
<TabItem value="project-methods" label="Project Management">
...
### **Issue Creation with Auto Project Creation**
...
<Tabs>
<TabItem value="search-methods" label="Search & Find">
...
<TabItem value="update-methods" label="Update Operations">
...
async def create_issue(issue: IssueCreate, current_user_id: int = 3):  # archon-agent
    """Create new issue with automatic project creation"""
    ...
    try:
        ...
        # Create issue with automatic project creation
        result = issue_manager.create_issue(
            ...
            auto_create_project=True          # AGENT-FRIENDLY
        )
...
@app.put("/issues/{issue_key}/status")
async def update_issue_status(issue_key: str, new_status: str, 
                             project_name: str, current_user_id: int = 3):
    """Update issue status with project verification"""
    
    issue_manager = IssueManager()
    
    try:
        success = issue_manager.update_issue_status(
            issue_key=issue_key,
        ...
        if success:
            return {"message": f"Issue {issue_key} status updated to {new_status}"}
        else:
...
new_issue = issue_manager.create_issue(
    ...
    project_name="AI Development Project",  # Will be created if doesn't exist
...
project_issues = issue_manager.get_issues_by_project("AI Development Project")
print(f"Found {len(project_issues)} issues in project")

# Find issues by task ID
task_issues = issue_manager.get_issues_by_task_id("TASK-456")
print(f"Found {len(task_issues)} issues linked to task TASK-456")

# Update issue status
success = issue_manager.update_issue_status(
    issue_key=new_issue['issue_key'],
    new_status="in_progress",
    user_id=3,  # archon-agent
...
### **Before Creating Issues**
- [ ] Validate project exists or use `auto_create_project=True`
- [ ] Set current user ID for audit trail
- [ ] Include task ID for linking if applicable
...
- [ ] Use `get_or_create_project()` for agent operations
- [ ] Include audit trail user context
- [ ] Validate input parameters
- [ ] Handle database transactions properly
- [ ] Log integration activities

The Issue Management API provides a robust foundation for integrating issue tracking into your applications while supporting both human users and AI agents with automatic project creation and comprehensive audit trails.
