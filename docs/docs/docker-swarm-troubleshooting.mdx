...
```bash
# Generate join token on swarm manager (10.202.70.20)
docker swarm join-token worker

# On new node, join swarm (replace <manager-ip> with 10.202.70.20)
docker swarm join --token <token> 10.202.70.20:2377
...
|------|------------|------|--------|
| **hl-dswarm1** | `10.202.70.20` | Manager | Active |
| **hl-dswarm2** | `10.202.70.21` | Worker | Active |
| **hl-dswarm3** | `10.202.70.22` | Worker | Active |
